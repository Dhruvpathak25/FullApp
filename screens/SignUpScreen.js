...
const SignUpScreen = ({navigation}) => {
  ...
  const validation = async () => {
    ...
    if (isValid) {
      try {
        // Retrieve existing user data from AsyncStorage using the same key
        const existingUserData = await AsyncStorage.getItem('userData');
        let userList = existingUserData ? JSON.parse(existingUserData) : [];

        // Check if the email is already registered
        const isEmailExists = userList.some(user => user.email === email);
        if (isEmailExists) {
        ...
        // Save new user data to the list
        const newUser = {fullName, email, password};
        userList.push(newUser);

        // Save the updated user list to AsyncStorage with the same key
